...
from app.scoring import framework_score as _framework_score
...
def list_requirements(db: Session, framework_id: uuid.UUID) -> list[models.Requirement]:
    return db.query(models.Requirement).filter_by(framework_id=framework_id).all()


def list_controls(db: Session, framework_id: uuid.UUID) -> list[models.Control]:
    return (
...
def list_controls_with_status(
    db: Session, framework_id: uuid.UUID, organization_id: uuid.UUID
) -> list[dict]:
    controls = list_controls(db, framework_id)
...
def get_framework_score(db: Session, framework_id: uuid.UUID, organization_id: uuid.UUID) -> dict:
    fw = db.get(models.Framework, framework_id)
    if fw is None:
        raise NotFoundError("Framework not found")
    result = _framework_score(db, framework_id, organization_id)
    return {
        "framework_id": fw.id,
